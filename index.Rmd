...
corpus <- dplyr::bind_rows(join_big2019,join_big2020,join_big2021)
```
...
### `r shiny::icon("exclamation-triangle")` ***NEW!***<br>8. In the mood for which key?<br>Chord and Key estimations for "Mood"
...
<!-- ### `r shiny::icon("exclamation-triangle")` ***NEW!***<br>9. Frame 9 -->


<!-- # cps %>% -->
<!-- #   group_by(year...31) %>% -->
<!-- #   summarize(key) %>% -->
<!-- #   mutate(key_key = fct_recode(key, -->
<!-- #                               0 = "C", -->
<!-- #                               1 = "C♯", -->
<!-- #                               2 = "D", -->
<!-- #                               3 = "D♯", -->
...
p1 <- ggplot(sd_19, aes(x = valence, y = energy, colour = week...14, label = `Track Name`, label2 = Artist)) + geom_point(aes(size = danceability, alpha = 0.4, fill = week...14)) + theme_light() 
p2 <- ggplot(sd_19, aes(x = week...14, y = valence, colour = week...14, label = `Track Name`, label2 = Artist)) + geom_point(aes(alpha = 0.4)) + labs(x = "Week") + theme_light()
...
p <- ggplot(comb_data.long, aes(x = week_year, label = max_streams, label2 = min_streams, label3 = top_track)) +
  geom_line(aes(x = factor(week_year), y = max_streams/coeff, group = 1),
            alpha = 0.7, size = 1) +
  geom_point(aes(y = max_streams/coeff), group = 1) +
  geom_line(aes(x = factor(week_year), y = mean_streams/coeff, group = 2),
            alpha = 0.7, size = 1) +
  geom_point(aes(y = mean_streams/coeff), group = 2, alpha = 0.7) +
