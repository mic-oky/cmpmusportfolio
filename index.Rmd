...
flc_2 <- filter_checkbox(id = "week", label = "Week", sharedData = sd_corpus, group = ~week_n, inline = TRUE)
...
p1 <- ggplot(sd_corpus, aes(x = valence, y = energy, colour = pandemic, label = `Track Name`, label2 = Artist)) +
  ...
  labs(x = "Valence", y = "Energy", title = "Valence/Energy - Quadrants") +
  theme_light()
...
p2 <- ggplot(sd_corpus, aes(x = week, colour = pandemic, label = year, group = 1)) +
  ...
  labs(x = "Week", y = "Average Valence", title = "Valence over time") + theme_light()
...
p3 <- ggplot(sd_corpus, aes(x = week, colour = pandemic, label = year, group = 1)) +
  ...
  labs(x = "Week", y = "Average Streams", title = "Number of Streams over time") + theme_light()

anno <- list(x = 0, y = 1, text = "Angry", xref = "x", yref = "y", showarrow = FALSE)
anno2 <- list(x = 1, y = 1, text = "Happy", xref = "x", yref = "y", showarrow = FALSE)
anno3 <- list(x = 1, y = 0, text = "Relaxed", xref = "x", yref = "y", showarrow = FALSE)
anno4 <- list(x = 0, y = 0, text = "Sad", xref = "x", yref = "y", showarrow = FALSE)
...
bscols(widths = c(6,6),
       ggplotly(p1 + xlim(0,1) + ylim(0,1)) %>%
         layout(annotations = (arrow_up)) %>%
         layout(annotations = (arrow_down)) %>%
         layout(annotations = (arrow_right)) %>%
         layout(annotations = (arrow_left)) %>%
         layout(annotations = (anno)) %>%
         layout(annotations = (anno2)) %>%
         layout(annotations = (anno3)) %>%
         layout(annotations = (anno4)),
       list(
         ggplotly(p2),
         ggplotly(p3))
       )
...
The interesting variables to compare the different periods are *valence* and *energy* as these reflect the valance/arousal model that show *Happy*, *Angry*, *Sad*, *Relaxed* emotions. 
...
### 4. Top streamed tracks: Consistent hits and one hit wonders. **Difficult to classify most popular song**{data-commentary-width=500}  
...
top_tot_streams <- distinct(
  # traxjoin %>%
  comb_data_trax %>%
    mutate(sd_tot_streams = (sqrt((tot_streams-mean(tot_streams))^2)/(nrow(comb_data_trax)-1))) %>%
    group_by(top_track) %>% 
...
names(top_tot_streams_t)[3] <- "Std Dev - Total Streams"
names(top_tot_streams_t)[4] <- "Number of weeks at Top 1"
names(top_tot_streams_t)[5] <- "Average Streams per week at Top 1"
...
fls4_3 <- filter_slider("avg_streams", "Average Streams per week at Top 1", sd_top_tot, "Average Streams per week at Top 1", step = 100000)
...
miljoen_17c <- miljoen_17 %>%
  ...
  geom_tile() +
  labs(x = "Time (s)", y = NULL, fill = "Magnitude", title = "17 Miljoen Mensen (2020)", subtitle = "Chromagram") +
  theme_minimal() +
...
miljoen_15c <- miljoen_15 %>%
  ...
  geom_tile() +
  labs(x = "Time (s)", y = NULL, fill = "Magnitude", title = "15 Miljoen Mensen (1996)", subtitle = "Chromagram") +
  theme_minimal() +
  scale_fill_viridis_c()
...
miljoen_p <- compmus_long_distance(
  ...
  geom_tile() +
  coord_equal() +
  labs(x = "15 Milljoen mensen (1996)", y = "17 Miljoen mensen (2020)", title = "Dynamic Time Warp", subtitle = "17 miljoen mensen vs. 15 mijoen mensen") +
  theme_minimal() +
...
grob1 <- grobTree(
  textGrob("A1", x=0.145,  y=0.165, hjust=0,
           gp=gpar(col="White", fontsize=13, fontface="bold")))
grob2 <- grobTree(
  textGrob("B1", x=0.285,  y=0.305, hjust=0,
           gp=gpar(col="White", fontsize=13, fontface="bold")))
grob3 <- grobTree(
  textGrob("A2", x=0.425,  y=0.165, hjust=0,
           gp=gpar(col="White", fontsize=13, fontface="bold")))
grob4 <- grobTree(
  textGrob("B2", x=0.595,  y=0.305, hjust=0,
           gp=gpar(col="White", fontsize=13, fontface="bold")))
grob5 <- grobTree(
  textGrob("C", x=0.725,  y=0.75, hjust=0,
           gp=gpar(col="White", fontsize=13, fontface="bold")))
grob6 <- grobTree(
  textGrob("B3", x=0.855,  y=0.305, hjust=0,
           gp=gpar(col="White", fontsize=13, fontface="bold")))

ssm_p1 <- ssm_p + annotation_custom(grob1) + annotation_custom(grob2) + annotation_custom(grob3) +
  annotation_custom(grob4) + annotation_custom(grob5) + annotation_custom(grob6)

ssm_t1 <- ssm_t + annotation_custom(grob1) + annotation_custom(grob2) + annotation_custom(grob3) +
  annotation_custom(grob4) + annotation_custom(grob5) + annotation_custom(grob6)
...
##### Self Similarity
The second and third plots are Self Similarity Matrices (SSM); The first being pitch, and the second timbre. These plots show the structure of a track by denoting patterns of similarities that reoccur. Diagonal lines and a checkerboard pattern show similarity and repetition. The black lines indicates the repeated parts.

The track seems to follow the following pattern: ***A1 - B1 - A2 - B2 - C - B3***

The timbre SSM is plotted using Euclidean norm, Euclidean distance and summarized by the mean. The plot shows a faint checkerboard pattern which implies some form of repetition in the track. The red lines at the 150 second mark ***C*** there is a significant timbre difference. This is when the breakdown occurs with the earlier mentioned “Millenial Whoop”.

The pitch SSM is plotted using Euclidean norm, cosine distance and summarized by root mean square. This plot shows a slightly more noticeable checkerboard pattern. The 150 second mark ***C***, the plot shows a significant change.
...
<ul class="nav nav-tabs">
  <li class="nav-item">
    <a class="nav-link active" data-toggle="tab" href="#confmat">Confusion Matrix</a>
  </li>
  <li class="nav-item">
    <a class="nav-link" data-toggle="tab" href="#randomforest">Random Forest</a>
  </li>
  <li class="nav-item">
    <a class="nav-link" data-toggle="tab" href="#forestfeatures">Forest Features</a>
  </li>
</ul>

<div id="myTabContent" class="tab-content">
  <div class="tab-pane fade active in" id="confmat">
...
```
  ...
  <div class="tab-pane fade" id="randomforest">
...
```
  ...
  <div class="tab-pane fade" id="forestfeatures">

```{r, echo=FALSE}
corpus_features %>%
  ggplot(aes(x = Streams, y = G, colour = pandemic, size = c06)) +
