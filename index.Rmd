...
### `r shiny::icon("exclamation-triangle")` ***NEW!***<br>8. In the mood for which keys?<br>Chord and Key estimations for "Mood".
...
mk <- mood %>% 
  ...
  theme_minimal() +
  labs(x = "Time (s)", y = "", fill = "Magnitude", title = "Mood (feat. iann dior) -24kGoldn", subtitle = "Keygram")
...
mc <- mood %>% 
  ...
  scale_fill_viridis_c(guide = "none") +
  theme_minimal() +
  labs(x = "Time (s)", y = "", fill = "Magnitude", title = "Mood (feat. iann dior) -24kGoldn", subtitle = "Chordogram")
...
According to the [Spotify API](https://developer.spotify.com/documentation/web-api/reference/#object-audiofeaturesobject), this track is in the 7th key,  with mode 0: meaning G minor.

**Chordify**

The [Chordify algorithm](https://chordify.net/chords/24kgoldn-mood-official-video-ft-iann-dior-24kgoldnvevo) identified the chords the following (4/4) loop:
...
### `r shiny::icon("exclamation-triangle")` ***NEW!***<br>9. Historgram of Keys in the corpus.
...
2020 Contains the most data, in order to make more meaningful comparisons to 2019, more data is required.
2021 is still in progress, comparisons with the other years are less meaningful.
...
### 2. A trip down memory lane:<br>**What was life like before the pandemic?**<br><br>*Note: tentative*
...
p1 <- ggplot(sd_19, aes(x = valence, y = energy, colour = week...14, label = `Track Name`, label2 = Artist)) +
  ...
  theme_light() 
p2 <- ggplot(sd_19, aes(x = week...14, y = valence, colour = week...14, label = `Track Name`, label2 = Artist)) +
  geom_point(aes(alpha = 0.9)) + 
  geom_point(aes(y = mean_valence, alpha = 1)) + 
  geom_line(aes(x = week...14, y = mean_valence, group = 1)) +
...
comb_data.long <- melt(comb_data, id.vars = c("week", "year...13", "week_year", 
                                              "max_streams", "min_streams", "mean_streams", "top_track"),
                       measure.vars = c("deceased", "hospital_admission"))
...
p <- ggplot(comb_data.long, aes(x = week_year, label = max_streams, label2 = min_streams, label3 = top_track)) +
  geom_line(aes(x = factor(week_year), y = max_streams/coeff, group = 1, color = "Streams Top 1 track"),
            alpha = 0.9, size = 1) +
  geom_point(aes(y = max_streams/coeff), group = 1, alpha = 0.2, size = 1) +
  geom_line(aes(x = factor(week_year), y = mean_streams/coeff, group = 2, color = "Average Streams of Top 50"),
            alpha = 0.9, size = 1) +
  geom_point(aes(y = mean_streams/coeff), group = 2, alpha = 0.2, size = 1) +
  ...
  scale_color_manual(name = "Variables", values = c("Streams Top 1 track" = "lightblue4",
                                                    "Average Streams of Top 50" = "purple2")) +
...
- **c05* [Unknown]:** It has the highest magnitude at around 150 seconds, noticeable is the loudness of the "[Millenial Whoop](https://en.wikipedia.org/wiki/Millennial_whoop)".
